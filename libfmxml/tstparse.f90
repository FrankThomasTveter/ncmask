...
program tstparse
   ...
   do while ( ierr .eq. 0 )
      ...
      !
      if ( ierr .ne. 0 ) then
         !write(*,*) 'TSTPARSE Error reading file.'
         exit
      endif
      !
      ! Skip comments
      ...
      !
      !write(*,*) 'TSTPARSE Line:',trim(string)
...
subroutine check_xml_file
   ...
   !write(*,*) 'TSTPARSE Checking file:',title

   call xml_open( info, 'tstparse.xml', .true. )
   call xml_options( info, report_lun = 20, report_details = .true. )

   do
      call xml_get( info, tag, starttag, endtag, attribs, no_attribs, data, no_data )
      if ( xml_error(info) ) then
         write(*,*)  'Error ', title
         write(20,*) 'Error ', title
      endif

      write(20,* ) tag, starttag, endtag
      do i = 1,no_attribs
         write(20,'(i3,1x,3a)') &
   ...
   call xml_close( info )
   !write(*,*) 'TSTPARSE Checking file done'
