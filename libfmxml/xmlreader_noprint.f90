...
! reading routines.
!
! $Id: xmlreader.f90,v 1.21 2007/12/07 10:10:25 arjenmarkus Exp $
!
! TODO:
! - Private routines!
...
program xmlreader
   ...
   character(len=80)                      :: inittag
   logical                                :: starttag,endtag
...
!
   ...
   character(len=50), dimension(1:3,1:notypes_predefined) :: predefined_types
   integer                                :: notypes = notypes_predefined
   data ((predefined_types(i,j) , i=1,3), j=1,notypes_predefined ) / &
'logical'       ,'   logical'                        , 'read_xml_logical', &
'logical-1dim'  ,'   logical, dimension(:), pointer' , 'read_xml_logical_1dim',  &
...
'integer-1dim'  ,'   integer, dimension(:), pointer' , 'read_xml_integer_1dim',  &
...
'real-1dim'     ,'   real, dimension(:), pointer'    , 'read_xml_real_1dim', &
...
'word-1dim'     ,'   character(len=?), dimension(:), pointer' , 'read_xml_word_1dim',  &
...
'line-1dim'     ,'   character(len=?), dimension(:), pointer'  , 'read_xml_line_1dim',  &
...
'character-shape','   character(len=?), dimension(SHAPE)'      , 'read_xml_line_array' /
   ...
   call xml_get( info, inittag, starttag, endtag, attribs, noattribs, data, nodata )

   do
      call xml_get( info, tag, starttag, endtag, attribs, noattribs, data, nodata )
      write(20,*) 'tag: ',tag
...
subroutine write_prolog
  ...
  &   'subroutine init_xml_file_'//trim(fname)
   ...
   write( lumain, '(a)' ) &
      ...
      &   '   endif', &
      &   '   do', &
      &   '      call xml_get( info, inittag, starttag, endtag, attribs, noattribs, &', &
      &   '         data, nodata)', &
      &   '      if ( inittag .ne. ''!--'' ) exit', &
      &   '   enddo', &
      &   '   if ( inittag .ne. "' // trim(root_name) // '" ) then', &
...
subroutine add_begin_loop( checktag, component )
  ...
  &   '   has_dvar = .true.'
   ...
   if ( checktag ) then
      write( luloop, '(a)' ) &
  &   '      if ( endtag .and. tag .eq. inittag ) then'  ,&
  &   '         exit'                                     ,&
  &   '      endif'
   endif
   write( luloop, '(a)' ) &
...
subroutine add_typedef( strict, dyn_strings )
  ...
  &         '( info, inittag, endtag, attribs, noattribs, data, nodata, &' ,&
  &   '              dvar, has_dvar )'                              ,&
  &   '   type(XML_PARSE)                                 :: info'   ,&
  &   '   character(len=*), intent(in)                    :: inittag',&
  &   '   logical, intent(inout)                          :: endtag  ',&
  &   '   character(len=*), dimension(:,:), intent(inout) :: attribs',&
  &   '   integer, intent(inout)                          :: noattribs',&
  &   '   character(len=*), dimension(:), intent(inout)   :: data   ',&
...
subroutine add_placeholder( strict, dyn_strings )
  ...
  &         '( info, inittag, attribs, noattribs, data, nodata )'   ,&
  &   '   type(XML_PARSE)                                 :: info'   ,&
  &   '   character(len=*), intent(in)                    :: inittag',&
  &   '   character(len=*), dimension(:,:), intent(inout) :: attribs',&
  &   '   integer, intent(inout)                          :: noattribs',&
